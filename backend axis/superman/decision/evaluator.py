"""Probability core for the decision workspace.

`evaluate(action, region)` is the single source of truth. The TypeScript port in
`frontend/src/decision/evaluator.ts` must match numerically because both ingest
the same `state.json` action rows and `intel.json` region blobs.

See `docs/decision-engine.md` for the derivation.
"""
...
from superman.intel.events import EventCategory
from superman.intel.morale import RegionIntel
