"""Faction-leader adapters that emit `LeaderSignal` rows.

A tiny ABC hides two feeders:

- `StubAdapter` consumes `data/leader_statements.json` (deterministic demos and
  tests).
- `GDELTAdapter` hits the GDELT 2.0 GKG / Events API where `Actor1` matches the
  leader string, folding outputs through the same CAMEO → `LeaderSignalType`
  ladder.

Neither adapter mutates `Theater`. Callers append into `theater.leader_signals`
so `SnapshotExporter` serialises whatever arrived this tick.

Severity stays on `[-1, +1]`. The live adapter persists raw Goldstein/10 inside
`goldstein` when present.
"""
...
from superman.domain.political import LeaderSignal, LeaderSignalType


# CAMEO root code → LeaderSignalType mapping. Hundreds of granular codes funnel
# into Superman's six high-level buckets via the leading two-character prefix.
...
class GDELTAdapter(LeaderStatementAdapter):
    ...
    def _http_get_json(self) -> dict[str, Any]:
        ...
        req = Request(url, headers={"User-Agent": "superman-hackathon/0.5"})
...
def build_adapter(kind: str = "stub", **kwargs: Any) -> LeaderStatementAdapter:
    ...
    Mirrors the `superman.intel.pipeline.build_source` pattern.
    """
    if kind == "stub":
