"""Combat draws and detection scaffolding.

Seeded RNG keeps a batch reproducible once you hash the participating order ids.
Resolution dataclasses return pure payloads that `orders` merges into `Theater`.
Range and detection checks precede adjudication.

Model summary (demo simplification)
-----------------------------------
For attackers A versus defenders D:
...
Morale dips with strength loss; retreats trigger when thresholds trip. Air /
missile / naval strikes reuse a simplified one-sided shot with optional SAM work.
"""
...
from superman.domain.military_assets import IsrCoverage, MissileRange
