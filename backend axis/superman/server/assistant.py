...
Turns commander prose plus the live `Theater` into JSON order rows the UI can
stage. Stateless handler: tighten the world brief, invoke OpenAI with rigid
schema, funnel every suggestion through `OrderRegistry.decode` and
`Order.validate`. Bad rows land in `warnings` so partial plans still return.

Depends on `OPENAI_API_KEY` (otherwise 503). The brief trims long lists and
only surfaces hostile units/assets inside a permissive sensing bubble so the
model gets fog-of-war similar to what the SPA already sketches.
"""
...
from superman.server.world_edits import apply_edits
from superman.sim.orders import OrderBatch, OrderRegistry
...
SYSTEM_PROMPT = """\
You are the staff officer for the Superman C2 sim. You receive a high-level
...
def _call_openai(prompt: str, brief: dict[str, Any], model: str) -> dict[str, Any]:
    ...
    if not api_key:
        raise AssistantUnavailable(
            "OPENAI_API_KEY is not set. Export it before running `superman serve`."
        )
    try:
