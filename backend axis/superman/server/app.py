"""FastAPI surface for the live C2 theatre and order execution.
...
Payload layout matches `superman.sim.orders.OrderBatch`:
...
CORS is wide open on purpose: single-tenant demo only.
"""
...
def _load_dotenv_once() -> None:
    ...
    `python -m superman serve` or by uvicorn's reloader subprocess. Existing
...
from superman.ai import OpenAIScenarioDesigner, ScenarioDesignerError  # noqa: E402
from superman.decision.actions import DEFAULT_ACTIONS  # noqa: E402
from superman.decision.explain import build_explanation, region_from_dict  # noqa: E402
from superman.server.assistant import AssistantUnavailable, suggest_orders  # noqa: E402
from superman.server.state import TheaterStore, get_store  # noqa: E402
from superman.sim.orders import OrderBatch, OrderRegistry  # noqa: E402
...
def create_app(*, store: TheaterStore | None = None) -> FastAPI:
    app = FastAPI(title="Superman live theatre", version="0.1.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
        allow_methods=["*"],
