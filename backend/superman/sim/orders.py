"""Typed orders executed against `Theater` state.

Incoming batches deserialize into polymorphic `Order` subclasses. Each kind
implements validation plus application hooks so FE/HTTP callers stay thin.

Architecture notes:

- ABC `Order`; subclasses advertise a unique JSON `kind` string.
- `OrderRegistry` maps dict payloads onto concrete classes (self-registering).
- `OrderBatch.validate_all` runs before `apply_all`, grouped by deterministic
  phase ordering so illegitimate mutations never halfway apply while still
  letting move→strike→resupply arcs finish in order.

Combat lives in ``superman.sim.combat``; order classes translate parameters and
patch results back onto the aggregate.
"""
...
from superman.domain.country import Country, PublicOpinion
...
from superman.sim.combat import (
