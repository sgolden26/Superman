"""Typer façade for export/intel/serve plus settings helpers."""
...
from superman import scenarios
from superman.intel.morale_factors import (
    build_dataset as build_morale_factors_dataset,
    write_dataset as write_morale_factors_dataset,
)
from superman.intel.pipeline import IntelPipeline, build_source, default_region_ids
from superman.serialization.snapshot import SnapshotExporter
from superman.settings import (
...
# Using `__file__` anchors default paths to `<repo>/data/` no matter where
# you launch the CLI from (repo root vs `frontend/` vs elsewhere).
_PROJECT_ROOT = Path(__file__).resolve().parents[2]
_DATA_DIR = _PROJECT_ROOT / "data"

app = typer.Typer(add_completion=False, help="Superman C2 backend CLI.")
...
def serve(
    ...
    """Run the uvicorn-hosted API for interactive play.

    The dev frontend still targets `localhost:8000` and forwards `/api/*`
    ...
    import uvicorn  # local import keeps `superman export` cheap to start

    uvicorn.run(
        "superman.server.app:app",
        host=host,
        port=port,
        reload=reload,
        log_level="info",
