"""Row-level explainer for Decision workspace UI.

Given `(action, region, team)` return the dict the SPA renders under each
evaluator row: canonical `Outcome`, human `summary`, `math`, traceable
`sources`, plus political row payloads.

Structured as dicts only so `/api/decision/explain` never imports private types.

Public entrypoints:
    `build_explanation(theater, action, region, team) -> dict`
    `region_from_dict(d) -> RegionIntel`

Style:
- Narrative-first summaries; digits only when they change the story.
- `sources[].kind` ∈ {"intel_event", "leader_signal", "action_catalog",
  "scenario_assumption"}.
- `disclaimer` when the upstream data is synthetic.
"""
...
from superman.decision.actions import Action
...
from superman.domain.political import CredibilityTrack, LeaderSignal
from superman.domain.theater import Theater
from superman.intel.events import Event, EventCategory
from superman.intel.morale import Driver, RegionIntel
from superman.sim.political_engine import issuer_faction_id
