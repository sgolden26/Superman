"""Russia–Ukraine benchmark scenario (`eastern_europe`).

The identifier stays frozen for tooling that keys intel + decision payloads off it.
Roughly ~150 manoeuvre units plus infrastructure arcs, ISR sweeps, supply,
and UA admin-1 oblast metadata aligned with frontend border GeoJSON lookups.

Lon/lat are approximate OSS references; heavy geometry stays in
`frontend/public/borders/` with merge keys here.
"""
...
from superman.domain.countries import StubCountryRepository
from superman.domain.faction import Allegiance
from superman.domain.oblast import Oblast
from superman.domain.political import LeaderSignalType
