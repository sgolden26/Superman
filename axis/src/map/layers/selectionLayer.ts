...
export const SOURCE_SELECTION = "sm-selection";
export const LAYER_SELECTION_RETICLE = "sm-selection-reticle";

const ICON_ID = "sm-selection-reticle-icon";
const ICON_PX = 96;
...
/**
 * Renders the corner-bracket sprite for whichever map feature currently owns the
 * selection reticle source (`sm-selection`). When that GeoJSON source is empty,
 * nothing is painted.
 *
 * `icon-color` tints the SDF using each feature's `color` field (the active
 * team's faction tone). A soft halo around the glyph suggests emphasis without
 * baking glow into the raster itself.
 */
