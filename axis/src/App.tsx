...
export function App() {
  ...
  return (
    <div className="flex h-screen w-screen flex-col overflow-hidden bg-mil-900 text-mil-50">
      <HUD />
      <PoliticalStrip />
...
function LoadingOverlay() {
  return (
    <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
      <div className="seam border bg-mil-800/80 px-4 py-2 font-mono text-[10px] uppercase tracking-wider2 text-mil-100">
        loading theatre...
      </div>
    </div>
  );
...
function ErrorOverlay({ message }: { message: string }) {
  return (
    <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
      <div className="seam max-w-md border bg-mil-800/90 px-4 py-3 font-mono text-[11px] uppercase tracking-wider2 text-accent-danger">
        <div className="mb-1">scenario load failed</div>
        <div className="text-mil-100 normal-case">{message}</div>
        <div className="mt-2 text-mil-200 normal-case">
          run the backend exporter:
          <br />
          <span className="text-mil-50">
            cd "backend axis" &amp;&amp; python -m superman export --out ../data/state.json
          </span>
        </div>
      </div>
    </div>
