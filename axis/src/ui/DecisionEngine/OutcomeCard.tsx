...
export function OutcomeCard({
  ...
  const containerCls = roomy ? "seam-t px-5 py-5" : "seam-t px-4 py-3";
  const successHeaderCls = roomy
    ? "font-mono text-[14px] font-semibold uppercase tracking-wider2 text-mil-100"
    : "font-mono text-[10px] uppercase tracking-wider2 text-mil-200";
  const pctCls = roomy
    ? "font-mono text-[44px] font-semibold leading-none"
    : "font-mono text-[28px] leading-none";
  const helperCls = roomy
    ? "mt-2 font-mono text-[12px] font-medium normal-case text-mil-200"
    : "mt-1 font-mono text-[9px] normal-case text-mil-300";
  const groupHeaderCls = roomy
    ? "mb-1.5 font-mono text-[12px] font-semibold uppercase tracking-wider2 text-mil-200"
    : "mb-0.5 font-mono text-[9px] uppercase tracking-wider2 text-mil-300";
  ...
  const explanationCls = roomy
    ? "mt-4 border-l-2 border-mil-400 pl-3 text-[15px] font-medium leading-snug text-mil-50"
    : "mt-3 border-l-2 border-mil-400 pl-3 text-[12px] leading-snug text-mil-100";
  ...
  return (
    <div className={containerCls}>
      ...
      <div className={roomy ? "mt-3 flex flex-col gap-3" : "mt-2 flex flex-col gap-2"}>
        ...
        {political.length > 0 && (
          <div className={`rounded-sm border border-accent-amber/30 bg-mil-900/40 ${roomy ? "p-3" : "p-2"}`}>
...
function BreakdownRow({
  ...
  const labelTone =
    variant === "political"
      ? roomy ? "text-mil-50" : "text-mil-100"
      : roomy ? "text-mil-100" : "text-mil-200";
  ...
  const detailLineCls = roomy
    ? "mt-1 font-mono text-[11px] font-medium normal-case leading-snug text-mil-200"
    : "mt-0.5 font-mono text-[8px] normal-case leading-tight text-mil-300";
  const barTrackCls = roomy
    ? "mt-1.5 h-[5px] w-full bg-mil-600"
    : "mt-0.5 h-[3px] w-full bg-mil-600";
    ...
    <div>
      <button
        ...
        className={`w-full text-left ${
          interactive
            ? "cursor-pointer transition-colors hover:bg-mil-700/30"
            : "cursor-default"
        } ${expanded ? "bg-mil-700/40" : ""} -mx-1 rounded-sm ${buttonPad}`}
      ...
      >
        <div className={labelRowCls}>
          <span className={labelTone}>
            {interactive ? (
              <span
                className={`${roomy ? "mr-2 w-2.5 text-[14px]" : "mr-1.5 w-2"} inline-block text-mil-300`}
                aria-hidden="true"
              >
                {expanded ? "▾" : "▸"}
              </span>
...
function ExpandedDetail({
  ...
  return (
    <div className="seam-l ml-2 mt-1 border-mil-500/70 bg-mil-900/40 px-3 py-2">
      {loading && !detail && (
        <div className="font-mono text-[10px] text-mil-300">
          loading explanation…
        </div>
      )}
      {error && !detail && (
...
function DetailContent({ detail }: { detail: ExplanationRow }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <p className="text-[12px] leading-snug text-mil-50">{detail.summary}</p>
      {detail.math && (
        <div className="font-mono text-[10px] text-mil-300">
          <span className="text-mil-400">math · </span>
          {detail.math}
        </div>
      )}
      {detail.disclaimer && (
...
function PoliticalData({ detail }: { detail: ExplanationRow }) {
  ...
  if (detail.key === "pressure") {
    ...
    return (
      <div className="rounded-sm border border-accent-amber/30 bg-mil-900/30 p-2 text-[11px] text-mil-100">
        <div className="mb-1 font-mono text-[9px] uppercase tracking-wider2 text-mil-300">
          political pressure detail
        </div>
        <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 font-mono text-[10px] text-mil-200">
          <span>issuer</span>
          <span className="text-mil-50">
        ...
        {data.pressure_drivers && data.pressure_drivers.length > 0 && (
          <div className="mt-2">
            ...
            <ul className="mt-0.5 list-disc pl-4 text-[11px] text-mil-100">
        ...
        {data.wargame_note && (
          <div className="mt-2 border-l-2 border-accent-amber/40 pl-2 text-[10px] italic leading-snug text-mil-200">
            {data.wargame_note}
          </div>
        )}
      </div>
    ...
    return (
      <div className="rounded-sm border border-accent-amber/30 bg-mil-900/30 p-2 text-[11px] text-mil-100">
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 font-mono text-[10px] text-mil-200">
          ...
          <span className="text-mil-50">{formatSigned(imm)}</span>
          <span>resolve (long-range)</span>
          <span className="text-mil-50">{formatSigned(res)}</span>
        ...
        {data.gap_history && data.gap_history.length > 0 && (
          <div className="mt-2">
            ...
            <ul className="mt-0.5 flex flex-col gap-0.5 text-[10px] text-mil-100">
              {data.gap_history.map((h, i) => (
                <li key={`g-${i}`} className="font-mono">
                  <span className="text-mil-300">turn {h.turn} · </span>
                  said {formatSigned(h.signal_severity)} / did{" "}
                  {formatSigned(h.action_severity)}
                  <span className="text-mil-300"> · gap </span>
                  <span
                    className={
          ...
          <div className="mt-2">
            ...
            <ul className="mt-0.5 flex flex-col gap-1">
              {data.recent_signals.map((s) => (
                <li
                  key={s.id}
                  className="rounded-sm border border-mil-600/60 bg-mil-900/40 px-2 py-1 text-[11px] text-mil-100"
                >
                  <div className="font-mono text-[9px] uppercase tracking-wider2 text-mil-300">
...
function SourcesBlock({ sources }: { sources: ExplanationSource[] }) {
  return (
    <div>
      <div className="mb-0.5 font-mono text-[9px] uppercase tracking-wider2 text-mil-300">
        sources
      </div>
      <ul className="flex flex-col gap-1">
        {sources.map((s, idx) => (
          <li
            key={`src-${idx}`}
            className="rounded-sm border border-mil-600/60 bg-mil-900/40 px-2 py-1"
          >
            <div className="flex items-baseline justify-between gap-2">
              <div className="min-w-0 truncate text-[11px] text-mil-100">
                {s.label}
              </div>
              <span className="shrink-0 font-mono text-[8px] uppercase tracking-wider2 text-mil-400">
                {sourceKindLabel(s.kind)}
              </span>
            </div>
            {s.note && (
              <div className="mt-0.5 font-mono text-[9px] normal-case text-mil-300">
                {s.note}
              </div>
            )}
            {s.snippet && (
              <div className="mt-0.5 text-[10px] italic leading-snug text-mil-200">
                {s.snippet}
              </div>
            )}
            {s.url && (
