...
export function DecisionImmersive() {
  ...
  return createPortal(
    <div
      className="fixed bottom-5 left-0 right-0 top-0 z-[200000] flex flex-col border-b border-mil-600 bg-mil-900"
      role="dialog"
      aria-modal="true"
      aria-label="Decision workspace"
    >
      <header className="seam-b flex shrink-0 items-center justify-between gap-3 bg-mil-800 px-4 py-2">
      ...
      <div className="flex h-0 min-h-0 flex-1 flex-col overflow-hidden">
        {!scenario ? (
          <div className="px-4 py-8 text-center font-mono text-[11px] text-mil-200">
          ...
          <div ref={splitContainerRef} className="flex h-0 min-h-0 flex-1 flex-col overflow-hidden">
            <div className="flex h-0 min-h-0 flex-1 flex-row gap-0 overflow-hidden">
              <div className="w-[220px] shrink-0 overflow-y-auto border-r border-mil-600 bg-mil-800/80">
                <RegionList
              ...
              <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden p-2">
                {focusedRegion && resolvedEntity ? (
                  ...
                  <div className="p-4 font-mono text-[11px] text-mil-200">
            ...
            {outcome && scenario && resolvedEntity && (
              <>
                <div
                  ...
                  className="group flex h-3 shrink-0 cursor-ns-resize flex-col items-center justify-center gap-[3px] border-y border-mil-500 bg-mil-600 hover:bg-mil-500"
                  title="Drag to resize"
                >
                  <div className="h-0.5 w-20 rounded-full bg-mil-200 group-hover:bg-mil-50" />
                  <div className="h-0.5 w-20 rounded-full bg-mil-200 group-hover:bg-mil-50" />
                </div>
                <div
                  className="shrink-0 overflow-y-auto border-mil-600 bg-mil-800/95"
                  style={{ height: bottomHeight ?? "42vh" }}
                >
                  <div className="px-5 pt-3">
                    <div className="font-mono text-[12px] font-semibold uppercase tracking-wider2 text-mil-100">
            ...
            {!outcome && actions.length > 0 && !choseAction && (
              <div className="seam-t border-mil-600 bg-mil-800/60 px-4 py-2 font-mono text-[10px] text-mil-300">
