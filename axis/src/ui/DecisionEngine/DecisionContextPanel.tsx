...
export function DecisionContextPanel({
  ...
  const headerCls = roomy
    ? "mb-2.5 font-mono text-[13px] font-semibold uppercase tracking-wider2 text-mil-100"
    : "mb-1.5 font-mono text-[10px] uppercase tracking-wider2 text-mil-200";
  const bodyCls = roomy
    ? "grid gap-2 font-mono text-[13px] font-medium leading-snug text-mil-50"
    : "grid gap-1.5 font-mono text-[10px] leading-tight text-mil-100";
  const labelCls = roomy ? "text-mil-200" : "text-mil-300";
  const sepCls = roomy ? "text-mil-400" : "text-mil-300";
  const footnoteCls = roomy
    ? "mt-1.5 border-t border-mil-600/50 pt-2 font-mono text-[12px] font-medium text-mil-100"
    : "mt-0.5 border-t border-mil-600/50 pt-1.5 text-[9px] text-mil-200";

  return (
    <div className={`seam-b bg-mil-800/60 ${containerCls}`}>
      ...
      <div className={bodyCls}>
        <div className="flex flex-wrap gap-x-2 gap-y-0.5">
          ...
          <span className="text-mil-50">{scenario.current_turn}</span>
        ...
        <div className="flex flex-wrap gap-x-2 gap-y-0.5">
          ...
          {political.issuer_pressure != null ? (
            <>
              <span className="text-mil-50">{(political.issuer_pressure * 100).toFixed(0)}%</span>
        ...
        <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
          ...
          {sameSide ? (
            <span className="text-mil-200 normal-case" title="Evaluator uses issuer→adversary tracks">
            ...
            <>
              ...
              {political.bilateral_credibility_resolve != null && (
                <>
                  <span className={sepCls}>·</span>
                  <span className="text-mil-200">res {political.bilateral_credibility_resolve.toFixed(2)}</span>
                </>
              )}
            </>
          ) : (
