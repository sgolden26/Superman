...
function SourceLink({ url }: { url: string }) {
  return (
    <a
      ...
      className="inline-flex items-center gap-1 border border-mil-400 px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-wider2 text-mil-100 transition-colors hover:border-mil-200 hover:text-mil-50"
    >
...
function moraleTone(score: number): { color: string; label: string } {
  ...
  if (score >= 50) return { color: "var(--mil-50)", label: "moderate" };
  ...
  return (
    <div className="flex flex-col">
      <div className="seam-b px-4 pb-3 pt-4">
        <div className="flex items-center justify-between font-mono text-[10px] uppercase tracking-wider2 text-mil-200">
          <span>region morale</span>
          <span style={{ color: faction.color }}>{faction.name}</span>
        </div>
        <div className="mt-1 text-base font-semibold text-mil-50">
          {regionLabel}
        </div>
        ...
        <div className="mt-3 flex items-end justify-between gap-3">
          <div>
            ...
            >
              {score}
              <span className="ml-1 text-[14px] text-mil-200">/100</span>
            </div>
                ...
                <li key={d.event_id} className="flex items-start gap-2">
                  ...
                  <div className="flex-1 leading-snug">
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="text-[12px] text-mil-50">{d.headline}</div>
          ...
          {region.recent_events.slice(0, 3).map((e) => (
            <li key={e.id}>
              <button
                ...
                className="-mx-1 flex w-full flex-col items-stretch gap-0.5 rounded-sm px-1 py-1 text-left leading-snug transition-colors hover:bg-mil-700/40"
                title="Open article"
              >
                <div className="flex items-center gap-2 font-mono text-[9px] uppercase tracking-wider2 text-mil-200">
                  <span>{formatTimeAgo(e.ts, now)}</span>
                  <span className="text-mil-300">·</span>
                  <span>{CATEGORY_LABEL[e.category] ?? e.category}</span>
                  <span className="text-mil-300">·</span>
                  <span className="text-mil-300">{e.source}</span>
                </div>
                <div className="flex items-start justify-between gap-2">
                  <div className="text-[12px] text-mil-50">{e.headline}</div>
                  {e.url && <SourceLink url={e.url} />}
                </div>
                {e.snippet && (
                  <div className="mt-0.5 text-[11px] text-mil-200">{e.snippet}</div>
                )}
              </button>
            </li>
          ))}
