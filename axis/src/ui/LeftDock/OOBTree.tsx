...
export function OOBTree() {
  ...
  if (!scenario) {
    return (
      <div className="px-3 py-4 font-mono text-[10px] uppercase tracking-wider2 text-mil-200">
        warming up…
      </div>
    );
  }
  ...
  return (
    <div className="py-1">
      <div className="flex items-center justify-between px-3 pb-1 pt-2">
        ...
        <span className="font-mono text-[9px] tabular-nums text-mil-300">
      ...
      {groups.map(({ faction, domains }) => {
        ...
        return (
          <div key={faction.id} className="seam-soft-b">
            <button
            ...
            >
              ...
              />
              <span className="flex-1 truncate text-left font-mono text-[10px] uppercase tracking-wider2 text-mil-50">
                {faction.id}
              </span>
            </button>
            {factionOpen &&
                ...
                return (
                  <div key={domain}>
                    ...
                    >
                      ...
                      <span className="flex-1 truncate text-left font-mono text-[9px] uppercase tracking-wider2 text-mil-200">
                        {DOMAIN_LABEL[domain]}
                      </span>
                      <span className="font-mono text-[9px] tabular-nums text-mil-300">
                        {units.length}
                            ...
                            <li key={u.id}>
                              <button
                                ...
                                className={`flex w-full items-center gap-2 py-1 pl-10 pr-3 text-left font-mono text-[10px] tracking-wider2 transition-colors ${
                                  active
                                    ? "row-pick text-mil-50"
                                    : "text-mil-100 hover:bg-white/[0.02]"
                                }`}
                              >
