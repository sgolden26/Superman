...
export function Minimap({ className, variant = "default" }: MinimapProps) {
  ...
  const shell =
    className ??
    (variant === "pip"
      ? "relative h-40 w-56 overflow-hidden border border-[var(--seam-strong)] bg-mil-900/90 shadow-xl"
      : "absolute bottom-12 right-3 h-32 w-48 overflow-hidden border border-[var(--seam-strong)] bg-mil-900/90 shadow-xl");
  ...
  return (
    <div className={shell}>
      ...
      <div
        ref={boxRef}
        className="pointer-events-none absolute border border-mil-50/60"
      ...
      />
      <div className="absolute bottom-0 left-0 right-0 border-t border-[var(--seam)] bg-mil-900/85 px-2 py-0.5 font-mono text-[9px] uppercase tracking-wider2 text-mil-300 backdrop-blur-sm">
        minimap
      </div>
    </div>
  );
