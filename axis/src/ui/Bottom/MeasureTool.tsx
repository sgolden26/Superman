    ...
    <div className="flex items-center gap-2 px-3 py-1">
      <button
        ...
        className={`border px-2 py-0.5 font-mono text-[9px] uppercase tracking-wider2 transition-colors ${
          active
            ? "border-accent-amber bg-mil-700 text-accent-amber"
            : "border-mil-600 text-mil-200 hover:border-mil-400 hover:text-mil-50"
        }`}
        title="Measure ( M )"
      >
        ⌖ measure
      ...
      {active && (
        <>
          <span className="font-mono text-[10px] text-mil-100">
            {path.length === 0
              ? "click points on map"
          ...
          {path.length > 0 && (
            <button
              onClick={clear}
              className="font-mono text-[9px] uppercase tracking-wider2 text-mil-200 hover:text-mil-50"
            >
              clear
            </button>
          )}
