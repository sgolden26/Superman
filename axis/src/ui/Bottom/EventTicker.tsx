...
const CATEGORY_TONE: Record<IntelEvent["category"], string> = {
  ...
  nationalist_sentiment: "text-mil-100",
};
...
export function EventTicker({ thin = false }: { thin?: boolean }) {
  ...
  return (
    <div
      className={
        thin
          ? "seam-t flex h-5 items-center gap-2 bg-mil-900/95 px-2"
          : "seam-t flex h-9 items-center gap-3 bg-mil-900/95 px-3"
      }
    >
      <button
        ...
        className={
          thin
            ? "shrink-0 font-mono text-[8px] uppercase tracking-wider2 text-mil-200 hover:text-mil-50"
            : "font-mono text-[10px] uppercase tracking-wider2 text-mil-200 hover:text-mil-50"
        }
        title="Pause ticker"
      >
        {paused ? "▶" : "‖"}
      ...
      <span
        className={
          thin
            ? "shrink-0 font-mono text-[8px] uppercase tracking-wider2 text-mil-300"
            : "shrink-0 font-mono text-[9px] uppercase tracking-wider2"
        }
      >
        <span className="text-mil-300">ticker</span>
        <span className="px-1 text-mil-300">/</span>
        <span className="text-mil-100">{intel?.source ?? "—"}</span>
      </span>
      <div className="relative min-w-0 flex-1 overflow-hidden">
        <div
          className={`flex gap-3 whitespace-nowrap text-mil-100 ${
        ...
        >
          {[...events, ...events].map((ev, i) => (
            <button
              ...
              className="flex shrink-0 items-center gap-2 hover:text-mil-50"
            >
              <span className={CATEGORY_TONE[ev.category]}>
                {CATEGORY_GLYPH[ev.category]}
              </span>
              <span className="text-mil-200" title={ev.region_id}>
                [{regionNameById.get(ev.region_id) ?? ev.region_id}]
              </span>
              <span>{ev.headline}</span>
              <span className="text-mil-300">· {ev.source}</span>
            </button>
          ))}
          {events.length === 0 && (
            <span className="text-mil-300">no signals on the wire</span>
          )}
        </div>
      </div>
    </div>
