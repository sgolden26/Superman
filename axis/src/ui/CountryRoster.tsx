...
export function CountryRoster() {
  ...
  return (
    <div className="absolute inset-x-0 top-0 z-30 flex max-h-[60vh] flex-col bg-mil-800 seam-b">
      <div className="seam-b flex items-center justify-between px-4 py-2">
        ...
        <div className="flex items-center gap-2">
          {compareCountries.length > 0 && (
            <button
              ...
              className="seam border px-2 py-1 font-mono text-[10px] uppercase tracking-wider2 text-mil-100 hover:text-mil-50"
            >
              clear
            </button>
          )}
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 overflow-y-auto">
          <div
            className="seam-b grid items-baseline px-4 py-1.5 font-mono text-[10px] uppercase tracking-wider2 text-mil-200"
          ...
          >
            {COLUMNS.map((c) => (
              <button
                ...
                className="text-left hover:text-mil-50"
              >
                {c.label}
          ...
          <ul>
            {sorted.map((c) => {
              ...
              return (
                <li
                  key={c.id}
                  className={`seam-b grid items-baseline px-4 py-2 text-[11px] hover:bg-mil-700 ${
                    inCompare ? "bg-mil-700/50" : ""
                  }`}
                  style={{ gridTemplateColumns: gridTemplate }}
                >
                  <button
                    onClick={() => handleSelectCountry(c.id)}
                    className="flex items-center gap-2 text-left text-mil-50 hover:text-accent-amber"
                  >
                  ...
                  <div className="text-right">
                    <button
                      ...
                      className={`seam border px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider2 ${
                        inCompare
                          ? "border-accent-amber text-accent-amber"
                          : "text-mil-200 hover:text-mil-50 disabled:opacity-40"
                      }`}
                    >
                      {inCompare ? "−" : "+"}
                    </button>
...
function AlertCell({ value }: { value: number }) {
  return (
    <div className="flex items-center gap-1">
      ...
      <span className="ml-1 font-mono text-[10px] text-mil-200">{value}/5</span>
    </div>
  );
}
...
function CredibilityCell({ value }: { value: number | null }) {
  ...
  return (
    <div className="flex items-center gap-2" title={`Outgoing immediate credibility (avg)`}>
      <div className="relative h-1 w-14 bg-mil-700">
        <div
        ...
        />
        <div className="absolute top-0 h-full w-px bg-mil-500/60" style={{ left: "50%" }} />
      </div>
...
function PercentCell({ value }: { value: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="h-1 w-12 bg-mil-700">
        <div
          className="h-full"
          style={{ width: `${pct}%`, background: "var(--mil-100)" }}
        />
      </div>
      <span className="font-mono text-[10px] text-mil-100">{pct}%</span>
    </div>
  );
}
...
function CompareView({ countries }: { countries: Country[] }) {
  return (
    <aside className="seam-l flex w-[420px] shrink-0 flex-col overflow-y-auto bg-mil-800/80">
      <div className="seam-b px-4 py-2 font-mono text-[10px] uppercase tracking-wider2 text-mil-200">
        comparison
      </div>
      ...
      >
        ...
        {countries.map((c) => (
          <span
            key={c.id}
            className="text-right font-mono text-[10px] uppercase tracking-wider2 text-mil-50"
            title={c.name}
          >
      ...
      {COMPARE_METRICS.map((metric) => {
        ...
        return (
          <div
            key={metric}
            className="grid items-baseline gap-2 border-b border-mil-700/40 px-4 py-2"
            style={{ gridTemplateColumns: `1fr ${"0.5fr ".repeat(countries.length)}` }}
          >
            <span className="font-mono text-[10px] uppercase tracking-wider2 text-mil-100">
              {label}
            ...
            {countries.map((c, i) => {
              ...
              return (
                <div key={c.id} className="text-right">
                  <div className="font-mono text-[10px] text-mil-50">
                    {metricDisplay(c, metric)}
                  </div>
                  <div className="ml-auto h-1 w-full bg-mil-700">
                    <div
                      className="h-full"
                      style={{
                        width: `${values[i] * 100}%`,
