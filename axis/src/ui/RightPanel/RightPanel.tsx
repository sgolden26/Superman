...
export function RightPanel() {
  ...
  if (!open) {
    return (
      <div className="seam-l flex h-full w-7 shrink-0 items-start justify-center bg-mil-800 pt-2">
        <button
          onClick={() => setOpen(true)}
          className="font-mono text-[10px] uppercase tracking-wider2 text-mil-200 hover:text-mil-50"
  ...
  return (
    <aside className="seam-l deck-surface flex h-full w-[400px] shrink-0 flex-col">
      <div className="seam-b flex items-stretch">
        {TABS.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            ...
            className={`relative flex-1 px-3 py-2.5 font-mono text-[11px] uppercase tracking-wider2 transition-colors ${
              tab === t.id
                ? "text-mil-50"
                : "text-mil-200 hover:text-mil-50"
            }`}
          >
            {t.label}
            <span
        ...
        <button
          ...
          className="px-3 py-2 font-mono text-[11px] uppercase tracking-wider2 text-mil-300 transition-colors hover:text-mil-50"
          title="Collapse ( ] )"
        >
          ›
        </button>
