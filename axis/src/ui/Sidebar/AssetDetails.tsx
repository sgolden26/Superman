  ...
  return (
    <div className="seam-b px-4 pb-3 pt-4">
      <div className="font-mono text-[9px] uppercase tracking-wider2 text-mil-300">
        {parts.map((p, i) => (
          <span key={i}>
            {i > 0 && <span className="px-1 text-mil-400">/</span>}
            {p}
          </span>
        ))}
      </div>
      <div className="mt-1 text-[18px] font-semibold leading-tight tracking-tight text-mil-50">
        {name}
      </div>
    ...
    <>
      ...
      <ul className="px-4 py-2 space-y-1">
        {actions.map((a) => (
          <li
            key={a}
            className="font-mono text-[11px] uppercase tracking-wider2 text-mil-200"
          >
            — {a.replace(/_/g, " ")}
          </li>
        ))}
        ...
        <>
          ...
          <ul className="px-4 py-2 space-y-1">
            {base.home_port_for.map((u) => (
              <li
                key={u}
                className="font-mono text-[11px] uppercase tracking-wider2 text-mil-100"
              >
                {u}
              </li>
            ))}
          ...
          return (
            <li key={u.id}>
              <button
                ...
                className="seam w-full rounded-sm border border-mil-500 bg-mil-700/40 px-2 py-1 text-left transition-colors hover:border-mil-300"
              >
                <div className="flex items-center gap-1.5">
                  {f && <FactionTag faction={f} />}
                  <span className="truncate font-mono text-[11px] uppercase tracking-wider2 text-mil-100">
                    {u.name}
                  </span>
                </div>
                <div className="mt-0.5 font-mono text-[9px] uppercase tracking-wider2 text-mil-300">
    ...
    <div className="flex h-full flex-col">
      ...
      {frontline.notes && (
        <>
          ...
          <div className="px-4 py-3 font-mono text-[11px] leading-relaxed text-mil-100">
            {frontline.notes}
          </div>
        </>
      )}
