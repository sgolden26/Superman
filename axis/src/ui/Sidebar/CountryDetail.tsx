...
export function CountryDetail({ country, faction }: Props) {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="seam-b px-4 pb-3 pt-4">
        <div className="font-mono text-[9px] uppercase tracking-wider2 text-mil-300">
          country dossier
          <span className="px-1 text-mil-400">/</span>
          {country.iso_a3}
        </div>
        <div className="mt-1 flex items-baseline gap-2">
          <span className="text-2xl leading-none">{country.flag_emoji}</span>
          <span className="text-[18px] font-semibold leading-tight tracking-tight text-mil-50">
            {country.name}
          </span>
        </div>
        <div className="mt-1 text-[11px] text-mil-200">{country.official_name}</div>
    ...
    <div>
      ...
      {country.capital_city_id ? (
        <button
          ...
          className="flex w-full items-baseline justify-between px-4 py-2 text-left hover:bg-mil-700"
        >
          <span className="font-mono text-[11px] uppercase tracking-wider2 text-mil-100">
            {country.capital_city_id}
          </span>
          <span className="font-mono text-[10px] uppercase tracking-wider2 text-mil-200">
            jump →
          </span>
        </button>
      ) : (
        <div className="px-4 py-2 font-mono text-[11px] text-mil-200">no capital recorded</div>
      )}
      ...
      {g.cabinet.length === 0 ? (
        ...
        <ul className="px-4 py-2">
          {g.cabinet.map((m) => (
            <li
              key={m.title}
              className="flex items-baseline justify-between border-b border-mil-700/60 py-1.5 text-[11px]"
            >
              <span className="font-mono uppercase tracking-wider2 text-mil-200">
                {m.title}
              </span>
              <span className="text-mil-50">{m.name}</span>
            </li>
          ))}
        </ul>
      )}
...
function MilitaryTab({ country }: { country: Country }) {
  ...
  return (
    <div>
        ...
        <div className="flex items-center gap-1">
          {[1, 2, 3, 4, 5].map((n) => (
            <span
              ...
              style={{
                background:
                  n <= m.alert_level ? "var(--accent-amber)" : "var(--mil-600, #2a2f3a)",
              }}
            />
          ))}
        </div>
      ...
      <div className="px-4 py-3 text-[11px] leading-relaxed text-mil-100">
        {m.doctrine}
      </div>
      ...
      <div className="px-2 py-1">
        {m.branches.map((b) => (
          <details
            key={b.name}
            className="seam mb-2 border bg-mil-800/40"
          >
            <summary className="cursor-pointer list-none px-3 py-2">
              <div className="flex items-baseline justify-between">
                <span className="text-[11px] text-mil-50">{b.name}</span>
                <span className="font-mono text-[10px] uppercase tracking-wider2 text-mil-200">
            ...
            {b.inventory.length === 0 ? (
              <div className="px-3 py-2 font-mono text-[10px] uppercase tracking-wider2 text-mil-200">
              ...
              <ul className="px-3 py-2">
                {b.inventory.map((line) => (
                  <li
                    ...
                    className="grid grid-cols-[1fr_auto_auto] items-baseline gap-2 border-b border-mil-700/60 py-1 text-[11px]"
                  >
                    <span className="truncate text-mil-50">{line.label}</span>
                    <span className="font-mono text-mil-100">
                      {line.count.toLocaleString("en-GB")}
                    </span>
                    <span className="font-mono text-[9px] uppercase tracking-wider2 text-mil-200">
      ...
      {m.c2_nodes.length === 0 ? (
        ...
        <ul className="px-4 py-2">
          {m.c2_nodes.map((n) => (
            <li
              key={n}
              className="flex items-center gap-2 py-1 font-mono text-[11px] uppercase tracking-wider2 text-mil-50"
            >
              <span className="inline-block h-px w-3 bg-mil-300" />
              {n}
            </li>
          ))}
        </ul>
      ...
      {n.delivery_systems.length === 0 ? (
        ...
        <ul className="px-4 py-2">
          {n.delivery_systems.map((d) => (
            <li
              key={d}
              className="flex items-center gap-2 py-1 text-[11px] text-mil-50"
            >
              <span className="inline-block h-px w-3 bg-mil-300" />
              {d}
            </li>
...
function CompositionBars({ items }: { items: Composition[] }) {
  ...
  return (
    <div className="px-4 py-2">
      <div className="flex h-2 w-full overflow-hidden bg-mil-700">
        {items.map((c, i) => (
          <span
            key={c.label}
      ...
      <ul className="mt-2">
        {items.map((c, i) => (
          ...
          >
            <span className="flex items-center gap-2 text-mil-100">
              <span
            ...
            <span className="font-mono text-[10px] text-mil-200">
    ...
    <div>
      ...
      {d.alliance_memberships.length === 0 ? (
        ...
        <div className="flex flex-wrap gap-1.5 px-4 py-3">
          {d.alliance_memberships.map((m) => (
            <span
              key={m}
              className="seam border px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider2 text-mil-50"
            >
              {m}
            </span>
          ))}
      ...
      <ul className="px-2 py-1">
        {d.treaties.map((t) => (
          <li
            key={t.name}
            className="seam mb-1 border px-3 py-2"
          >
            <div className="flex items-baseline justify-between">
              <span className="text-[11px] text-mil-50">{t.name}</span>
              <span className="font-mono text-[9px] uppercase tracking-wider2 text-mil-200">
                {t.kind}
              </span>
            </div>
            <div className="mt-1 flex items-center justify-between font-mono text-[10px] text-mil-200">
              <span>parties: {t.parties.join(", ") || "—"}</span>
              <span style={{ color: t.in_force ? "var(--accent-ok)" : "var(--mil-300)" }}>
      ...
      <ul>
        {d.relations.map((r) => {
          ...
          return (
            <li key={r.other_country_id} className="border-b border-mil-700/40">
              <button
                disabled={!known}
                onClick={() => known && onSelectCountry(r.other_country_id)}
                className={`flex w-full flex-col gap-1 px-4 py-2 text-left ${
                  known ? "hover:bg-mil-700" : "cursor-default opacity-70"
                }`}
              >
                <div className="flex items-baseline justify-between">
                  <span className="flex items-center gap-2 text-[11px] text-mil-50">
                    {other?.flag_emoji}
                ...
                <div className="relative h-1 w-full bg-mil-700">
                  <span
                  ...
                  />
                  <span className="absolute left-1/2 top-0 h-full w-px bg-mil-400/60" />
                </div>
              </button>
            </li>
          );
    ...
    <div>
      ...
      <ul className="px-4 py-2">
        {g.land_borders.map((b) => {
          ...
          return (
            ...
            >
              <span className="text-mil-100">{label}</span>
              <span className="font-mono text-mil-200">
      ...
      <ul className="px-2 py-1">
        {g.key_bases.map((k) => (
          ...
          >
            <div className="flex items-baseline justify-between">
              <span className="text-[11px] text-mil-50">{k.name}</span>
              <span className="font-mono text-[9px] uppercase tracking-wider2 text-mil-200">
                {k.kind.replace(/_/g, " ")}
              </span>
            </div>
            <div className="mt-1 font-mono text-[10px] text-mil-200">
  ...
  return (
    <div>
      ...
      <div className="px-3 py-2">
        <p className="mb-2 text-[11px] leading-relaxed text-mil-200">
        ...
        <div className="grid grid-cols-1 gap-1.5">
          {country.available_actions.map((a) => (
            <button
              ...
              className="seam border px-3 py-1.5 text-left font-mono text-[11px] uppercase tracking-wider2 text-mil-200 opacity-70"
            >
              {a.replace(/_/g, " ")}
            </button>
          ))}
...
function Empty({ children }: { children: React.ReactNode }) {
  return (
    <div className="px-4 py-3 font-mono text-[11px] text-mil-200">{children}</div>
  );
}
