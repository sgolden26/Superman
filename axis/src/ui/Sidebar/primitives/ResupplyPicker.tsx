...
export function ResupplyPicker({ side, unitId, depotId }: Props) {
  ...
  return (
    <div className="col-span-2">
      ...
      {open && (
        <div className="seam-t mt-1 flex flex-col gap-2 px-1 py-2">
          {candidates.length === 0 ? (
            <div className="font-mono text-[10px] text-mil-200">
              {side === "from_unit"
                ? "no friendly depot available"
                : "no friendly unit available"}
            </div>
          ) : (
            <ul className="max-h-40 overflow-y-auto border border-[var(--seam)]">
              {candidates.map((c) => {
                    ...
                    <button
                      ...
                      className={`flex w-full items-center justify-between gap-2 px-2 py-1.5 text-left transition-colors ${
                        active
                          ...
                          : "text-mil-100 hover:bg-mil-700/40"
                      }`}
                    >
                      <span className="truncate font-mono text-[11px]">{c.name}</span>
                      <span className="shrink-0 font-mono text-[9px] uppercase tracking-wider2 text-mil-300">
                        {c.hint}
                      </span>
                    </button>
                  </li>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="seam border border-mil-500 px-2 py-1 font-mono text-[9px] uppercase tracking-wider2 text-mil-200 transition-colors hover:border-mil-300 hover:text-mil-50"
            >
              Cancel
            </button>
          </div>
