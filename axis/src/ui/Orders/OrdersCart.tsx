  ...
  return (
    <div className="pointer-events-none absolute right-3 top-3 z-[60] flex w-80 flex-col items-end gap-2">
      <button
        ...
        className={`pointer-events-auto inline-flex items-center gap-2 seam border ${teamBorder} bg-mil-800/95 px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider2 shadow-lg transition-colors hover:bg-mil-700`}
      >
        <span className={`inline-block h-1.5 w-1.5 rounded-full ${teamDot}`} />
        <span className={teamColor}>orders</span>
        <span className="text-mil-100">·</span>
        <span className="text-mil-50">{orders.length}</span>
        <span className="text-mil-300">{cartOpen ? "▾" : "▸"}</span>
      </button>

      {cartOpen && (
        <div className="pointer-events-auto w-full seam border bg-mil-800/95 shadow-xl">
          <div className={`seam-b flex items-center justify-between px-3 py-2 ${teamColor}`}>
            ...
            <button
              ...
              className="font-mono text-[12px] text-mil-200 hover:text-mil-50"
            >
              ×
            </button>
          </div>
          ...
          {orders.length === 0 ? (
            <div className="px-3 py-3 font-mono text-[10px] uppercase tracking-wider2 text-mil-200">
              {meReady
          ...
          {error && (
            <div className="seam-t flex items-start gap-2 bg-accent-danger/10 px-3 py-2">
              ...
              <button
                ...
                className="font-mono text-[11px] text-accent-danger hover:text-mil-50"
              >
                ×
              </button>
            </div>
          ...
          <div className="seam-t flex flex-col gap-2 px-3 py-2">
            <div className="flex items-center justify-between font-mono text-[9px] uppercase tracking-wider2 text-mil-200">
              <span>
                you · {meReady ? <span className="text-accent-ok">ready</span> : <span className="text-mil-100">planning</span>}
              </span>
              <span>
                {otherTeam} · {otherReady ? <span className="text-accent-ok">ready</span> : <span className="text-mil-100">planning</span>}
              </span>
            </div>
            ...
            <div className="flex items-center justify-between gap-2">
              <button
                ...
                className="seam border border-mil-500 px-2 py-1 font-mono text-[9px] uppercase tracking-wider2 text-mil-200 transition-colors hover:border-mil-300 hover:text-mil-50 disabled:cursor-not-allowed disabled:opacity-40"
              >
                Clear
              </button>
              ...
              {bothReady ? (
                <button
                  ...
                  className="seam border border-accent-ok bg-accent-ok/10 px-3 py-1 font-mono text-[10px] uppercase tracking-wider2 text-accent-ok transition-colors hover:bg-accent-ok/20 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-accent-ok/10"
                >
                ...
                <button
                  ...
                  className="seam border border-mil-300 px-3 py-1 font-mono text-[10px] uppercase tracking-wider2 text-mil-100 transition-colors hover:border-mil-50 hover:text-mil-50 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  Unready
                </button>
              ) : (
...
function OrderRow({ order, unitsById, factionsById, onSelect, onRemove, disabled }: OrderRowProps) {
  ...
  return (
    <li className="seam-b last:border-b-0 flex items-start gap-2 px-3 py-2">
      <button
        type="button"
        onClick={onSelect}
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {isLlm && (
            <span
              className="seam border border-accent-ok bg-accent-ok/10 px-1 py-px font-mono text-[8px] uppercase tracking-wider2 text-accent-ok"
              title="Suggested by the assistant"
            >
              AI
            </span>
          )}
          <span className="text-[12px] font-semibold text-mil-50 truncate">{title}</span>
        </div>
        <div className="mt-0.5 flex items-baseline gap-2 font-mono text-[9px] uppercase tracking-wider2 text-mil-200">
          {subtitle}
        </div>
      </button>
      <button
        ...
        className="shrink-0 seam border border-mil-500 px-1.5 py-0.5 font-mono text-[11px] text-mil-200 transition-colors hover:border-accent-danger hover:text-accent-danger disabled:cursor-not-allowed disabled:opacity-40"
      >
        ×
      </button>
    </li>
...
function ExecutionDigest({ outcomes, political }: DigestProps) {
  ...
  return (
    <div className="pointer-events-auto w-full seam border border-mil-500 bg-mil-800/95 p-2 shadow-xl">
      <div className="font-mono text-[9px] uppercase tracking-wider2 text-mil-200">
        last execute
      </div>
      {combat.length === 0 && polEntries.length === 0 && (
        <div className="font-mono text-[10px] text-mil-100">no kinetic effects</div>
      )}
      {combat.slice(0, 3).map((o) => (
        <div key={o.order_id} className="font-mono text-[10px] text-mil-100">
          · {o.kind} · {summariseOutcome(o)}
        </div>
      ))}
      {polEntries.length > 0 && (
        <div className="mt-1 font-mono text-[9px] uppercase tracking-wider2 text-mil-200">
          political:
        </div>
      )}
      {polEntries.slice(0, 4).map(([country, deltas]) => (
        <div key={country} className="font-mono text-[10px] text-mil-100">
