...
/** Compact theatre prompt anchored toward the lower centre (~720px): operators
 *  describe the round in prose and the assistant proposes aligned orders.
 *
 * Surfaces seam styling consistent with other chrome. It locks out while the
 * player's team has committed readiness or while a round run is in flight.
 * Cmd/Ctrl+Enter commits text; Esc clears any rationale banner.
 */
export function AssistantBar() {
  ...
  return (
    <div className="pointer-events-none absolute bottom-24 left-1/2 z-[70] flex w-[min(720px,90vw)] -translate-x-1/2 flex-col items-stretch gap-2">
      ...
      <div
        className={`pointer-events-auto flex items-end gap-2 seam border ${teamBorder} bg-mil-800/95 px-3 py-2 shadow-xl`}
      >
        <span
        ...
        <textarea
          ...
          className="flex-1 resize-none bg-transparent font-mono text-[12px] leading-tight text-mil-50 placeholder:text-mil-300 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
        />
        <button
          type="button"
          onClick={() => void send()}
          disabled={disabled || text.trim().length === 0}
          className={`shrink-0 seam border px-2 py-1 font-mono text-[10px] uppercase tracking-wider2 transition-colors disabled:cursor-not-allowed disabled:opacity-40 ${
            busy
              ? "border-mil-300 text-mil-100"
...
function RationaleStrip({
  ...
  return (
    <div className="pointer-events-auto seam border border-mil-500 bg-mil-800/95 px-3 py-2 shadow-xl">
        ...
        <div className="flex-1 space-y-1">
          ...
          {!error && rationale && (
            <>
              <div className="flex flex-wrap items-center gap-2">
                <span className="seam border border-accent-ok bg-accent-ok/10 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-wider2 text-accent-ok">
                  AI plan
                </span>
                ...
                {spawnedCount > 0 && (
                  <span className="seam border border-accent-amber bg-accent-amber/10 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-wider2 text-accent-amber">
                    deployed {spawnedCount} platform{spawnedCount === 1 ? "" : "s"}
                  </span>
                )}
              </div>
              <p className="text-[12px] leading-snug text-mil-50">{rationale}</p>
            </>
          )}
          ...
          {warnings.length > 0 && (
            <ul className="mt-1 space-y-0.5">
              ...
              {warnings.length > 6 && (
                <li className="font-mono text-[9px] uppercase tracking-wider2 text-mil-300">
