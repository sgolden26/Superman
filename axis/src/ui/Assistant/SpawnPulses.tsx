...
function Pulse({ pulse }: { pulse: SpawnPulse }) {
  ...
  return (
    <div style={{ position: "absolute", left: pos[0], top: pos[1] }}>
      ...
      />
      <div
        className="absolute animate-spawnLabel rounded-sm border bg-mil-900/85 px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider2 shadow-md"
        style={{
          left: 0,
          top: 0,
          borderColor: colour,
      ...
      >
        ...
        <span className="text-mil-50">{pulse.name}</span>
      </div>
    </div>
  );
}
