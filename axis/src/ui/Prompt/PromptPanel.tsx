...
export function PromptPanel() {
  ...
  return (
    <div className="flex h-full min-h-0 w-full min-w-0 flex-col bg-mil-800">
      <div className="seam-b flex shrink-0 items-center justify-between px-4 py-2">
      ...
      <div className="h-0 min-h-0 flex-1 overflow-y-auto">
        <form
          className="seam-b flex flex-col gap-3 bg-mil-800 px-4 py-3"
          onSubmit={onSubmit}
        >
          <label className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              rows={5}
              className="seam w-full resize-y rounded-none border bg-mil-900 px-2 py-1.5 font-mono text-[11px] leading-snug text-mil-50 outline-none placeholder:text-mil-300 focus:border-accent-amber"
            />
            <span className="font-mono text-[9px] text-mil-300">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <label className="flex flex-col gap-1">
              ...
              <input
                ...
                className="seam w-full rounded-none border bg-mil-900 px-2 py-1 font-mono text-[11px] text-mil-50 outline-none focus:border-accent-amber"
              />
            </label>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              type="submit"
              disabled={!canSubmit}
              className="seam border bg-mil-900 px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider2 text-mil-50 hover:border-accent-amber disabled:opacity-40 disabled:hover:border-mil-500"
            >
            ...
            {(result || error) && (
              <button
                type="button"
                onClick={onClear}
                className="seam border bg-mil-800 px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider2 text-mil-200 hover:text-mil-50"
              >
                clear
              </button>
            )}
            {generatedAt && !loading && (
              <span className="ml-auto font-mono text-[9px] text-mil-300">
          ...
          {error && (
            <div className="seam border border-accent-danger bg-mil-900 px-2 py-1.5 font-mono text-[10px] leading-snug text-accent-danger">
              {error}
            </div>
          )}
        </form>
        ...
        {!result && !loading && (
          <div className="px-4 py-6 font-mono text-[10px] leading-relaxed text-mil-300">
          ...
          <Field label="credibility">
            <div className="grid gap-0.5">
              {scenario.credibility.map((c, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 font-mono text-[10px] text-mil-100"
                >
          ...
          <Field label="leader signals">
            <div className="grid gap-1.5">
              {scenario.leader_signals.map((s) => (
                <div
                  key={s.id}
                  className="seam border bg-mil-900/60 px-2 py-1.5"
                >
                  <div className="flex flex-wrap items-center gap-1.5 font-mono text-[9px] uppercase tracking-wider2 text-mil-300">
                    <span className="text-mil-50">{s.speaker_faction_id}</span>
                    <span>·</span>
                    <span>{s.type}</span>
                    {s.target_faction_id && (
                      <>
                    ...
                    <span className="ml-auto text-mil-50">
                      {fmtSigned(s.severity)}
                    </span>
                  </div>
                  <p className="mt-1 font-mono text-[10px] leading-snug text-mil-100">
                    {s.text}
                  </p>
                </div>
              ))}
          ...
          <Field label="victory conditions">
            <div className="grid gap-1">
              {Object.entries(scenario.victory_conditions).map(([k, v]) => (
                <div key={k} className="font-mono text-[10px] text-mil-100">
                  <span className="uppercase text-mil-300">{k}</span>
...
function Section({
  ...
  return (
    <div className="seam-b bg-mil-800/60 px-4 py-3">
      <div className="mb-2 font-mono text-[10px] uppercase tracking-wider2 text-mil-200">
        {title}
      </div>
...
function Field({
  ...
  return (
    <div>
      ...
      <div className="font-mono text-[10px] leading-snug text-mil-100">
        {children}
      </div>
    </div>
  );
...
function KeyVal({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="font-mono text-[11px] text-mil-50">{value}</div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {items.map((it) => (
        <span
          key={it}
          className="seam border bg-mil-900/80 px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-wider2 text-mil-100"
        >
          {it}
        </span>
      ))}
...
function PressureRow(props: {
  ...
  return (
    <div className="seam border bg-mil-900/60 px-2 py-1.5">
      <div className="flex items-center gap-2 font-mono text-[10px] text-mil-50">
        <span className="text-mil-100">{props.id}</span>
        <span className="ml-auto font-mono text-[10px] text-mil-50">
          {(pct * 100).toFixed(0)}%
        </span>
        {props.deadline != null && (
          <span className="font-mono text-[9px] uppercase tracking-wider2 text-mil-300">
      ...
      <div className="mt-1 h-1 w-full bg-mil-700">
        <div
      ...
      {props.drivers && props.drivers.length > 0 && (
        <div className="mt-1 flex flex-wrap gap-1">
          {props.drivers.map((d, i) => (
            <span
              key={i}
              className="seam border bg-mil-800 px-1 py-0.5 font-mono text-[9px] text-mil-200"
            >
              {d}
            </span>
          ))}
