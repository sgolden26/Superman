...
export function PoliticalStrip() {
  ...
  return (
    <div className="seam-b flex h-9 shrink-0 items-stretch whitespace-nowrap bg-mil-800/95">
      <div className="flex shrink-0 items-center gap-3 px-4 font-mono text-[10px] uppercase tracking-wider2 leading-none text-mil-200">
        <span
          className="text-mil-100"
      ...
      <div className="seam-l flex min-w-0 flex-1 items-center gap-4 overflow-x-auto px-4 leading-none">
...
function FactionPressurePip({
  ...
  return (
    ...
    >
      ...
      {pressure.team_goal ? (
        <>
          <span className="text-mil-300">team goal ·</span>
          <span className={goalDeadlineTone}>
            {pressure.team_goal}
            {remaining != null ? ` T-${remaining}` : ""}
          </span>
          <span className="text-mil-300">·</span>
          <span className="text-mil-200">pressure</span>
          <div className="h-1.5 w-16 shrink-0 bg-mil-700">
            <div
              className="h-full"
              style={{ width: `${pct}%`, background: faction.color }}
            />
          </div>
          <span className="text-mil-100">{pct}%</span>
        </>
      ) : (
        <>
          <div className="h-1.5 w-24 shrink-0 bg-mil-700">
            <div
              className="h-full"
              style={{ width: `${pct}%`, background: faction.color }}
            />
