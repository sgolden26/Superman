...
export function HoverCard() {
  ...
  return (
    <div
      className="pointer-events-none absolute z-30 max-w-[260px] border border-[var(--seam-strong)] bg-mil-900/85 px-2.5 py-2 shadow-2xl backdrop-blur-md"
    ...
    >
      ...
      <div className="mt-1 truncate text-[12px] font-semibold tracking-tight text-mil-50">
        {card.title}
      </div>
      <div className="mt-1.5 grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5">
        {card.rows.map(([k, v]) => (
          <div key={k} className="contents">
            ...
            <span className="text-right font-mono text-[10px] tabular-nums text-mil-50">
              {v}
            </span>
          </div>
        ))}
